...
internal class WriteFileHandler : IntentHandler
{
        ...
        : base(context, IntentType.WriteFile, LockType.RequiresLock)
    {
        ...
        if (checksumHeader != null) _checksum = new Checksum(checksumHeader);
    ...
    internal override async Task Invoke()
    {
        ...
        var guardedStream = new ClientDisconnectGuardedReadOnlyStream(Request.Body,
        ...
        {
            await Response.Error((HttpStatusCode) 460,
                "Header Upload-Checksum does not match the checksum of the file");
            return;
        }

        if (guardedStream.CancellationToken.IsCancellationRequested) return;
    ...
    private Task WriteUploadLengthIfDefered()
    {
        ...
        if (uploadLenghtHeader != null && Store is ITusCreationDeferLengthStore creationDeferLengthStore)
            return creationDeferLengthStore.SetUploadLengthAsync(Request.FileId, long.Parse(uploadLenghtHeader),
    ...
    {
        if (!(Store is ITusConcatenationStore concatenationStore)) return Task.FromResult(false);
    ...
    {
        if (!(Store is ITusChecksumStore checksumStore)) return Task.FromResult(true);

        if (_checksum == null) return Task.FromResult(true);
