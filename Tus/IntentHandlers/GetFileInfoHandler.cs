...
internal class GetFileInfoHandler : IntentHandler
{
    ...
    internal override async Task Invoke()
    {
        ...
        if (!string.IsNullOrEmpty(uploadMetadata)) Response.SetHeader(HeaderConstants.UploadMetadata, uploadMetadata);
        ...
        if (Context.Configuration.Store is ITusConcatenationStore tusConcatStore)
        {
            ...
            if (uploadConcat is FileConcatFinal && uploadLength != uploadOffset) addUploadOffset = false;
        }

        if (addUploadOffset) Response.SetHeader(HeaderConstants.UploadOffset, uploadOffset.ToString());
