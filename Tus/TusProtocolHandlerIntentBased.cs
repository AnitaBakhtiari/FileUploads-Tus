...
internal static class TusProtocolHandlerIntentBased
{
    ...
    public static async Task<ResultType> Invoke(ContextAdapter context)
    {
        ...
        if (intentHandler == IntentHandler.NotApplicable) return ResultType.ContinueExecution;
        ...
        if (onAuhorizeResult == ResultType.StopExecution) return ResultType.StopExecution;
        ...
        if (intentHandler.LockType == LockType.RequiresLock)
        {
            ...
            if (!hasLock)
            {
                await context.Response.Error(HttpStatusCode.Conflict,
                    $"File {context.Request.FileId} is currently being updated. Please try again later");
                return ResultType.StopExecution;
            }
        }
        ...
        try
        {
            if (!await intentHandler.Validate()) return ResultType.StopExecution;
    ...
    private static FileConcat GetFileConcatenationFromIntentHandler(IntentHandler intentHandler)
    {
        return intentHandler is ConcatenateFilesHandler concatFilesHandler
            ? concatFilesHandler.UploadConcat.Type
            : null;
    }

    private static async Task<ResultType> VerifyTusVersionIfApplicable(ContextAdapter context,
