...
{
    private static readonly Lazy<Dictionary<Type, PropertyInfo>> _eventHandlers = new(GatherEventHandlers);

    internal static async Task<ResultType> Validate<T>(ContextAdapter context, Action<T> configure = null)
        where T : ValidationContext<T>, new()
    {
        ...
        if (eventContext.HasFailed)
        {
            ...
            if (eventContext is EventContext<AuthorizeContext>) includeTusResumableHeaderInResponse = false;
...
#pragma warning disable CS0618 // Type or member is obsolete
        if (context.Configuration.OnUploadCompleteAsync == null &&
            context.Configuration.Events?.OnFileCompleteAsync == null) return;

        var eventContext = FileCompleteContext.Create(context, configure);

        if (context.Configuration.OnUploadCompleteAsync != null)
            await context.Configuration.OnUploadCompleteAsync.Invoke(eventContext.FileId, eventContext.Store,
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    {
        if (context.Configuration.Events == null) return null;
