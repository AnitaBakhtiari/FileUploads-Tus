...
public static class ClientDisconnectGuard
{
    ...
    public static async Task<bool> ExecuteAsync(Func<Task> guardFromClientDisconnect,
    ...
    internal static async Task<ClientDisconnectGuardReadStreamAsyncResult> ReadStreamAsync(Stream stream, byte[] buffer,
    ...
    private static bool ClientDisconnected(Exception exception, CancellationToken cancellationToken)
    {
        ...
        if (exceptionFullName == "Microsoft.AspNetCore.Server.Kestrel.BadHttpRequestException") return true;
        ...
        if (exceptionFullName == "Microsoft.AspNetCore.Connections.ConnectionResetException") return true;
