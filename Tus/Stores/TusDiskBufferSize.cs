...
public class TusDiskBufferSize
{
    ...
    /// <summary>
    ///     Initializes a new instance of the<see cref="TusDiskBufferSize" /> class using the provided write buffer size and
    ///     <see cref="DefaultReadBufferSizeInBytes" />.
    /// </summary>
    /// <param name="writeBufferSizeInBytes">
    ///     The amount of data to keep in memory before flushing data to disk. A higher value
    ///     requires more RAM but uses less CPU and IO
    /// </param>
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="TusDiskBufferSize" /> class using the provided write and read buffer
    ///     sizes.
    /// </summary>
    /// <param name="writeBufferSizeInBytes">
    ///     The number of bytes to keep in memory before flushing data to disk. A higher value
    ///     requires more RAM but uses less CPU and IO
    /// </param>
    /// <param name="readBufferSizeInBytes">
    ///     The number of bytes to read at the time from the request. The lower the value, the
    ///     less data needs to be re-submitted on errors. However, the lower the value, the slower the operation is.
    /// </param>
    ...
    public static TusDiskBufferSize Default { get; } =
        new(DefaultWriteBufferSizeInBytes, DefaultWriteBufferSizeInBytes);
    ...
    {
        if (bufferSize <= 0) throw new TusConfigurationException($"{nameOfParameter} is zero or a negative number");
