    ...
    ITusCreationDeferLengthStore
{
    ...
    public TusDiskStore(string directoryPath, bool deletePartialFilesOnConcat) : this(directoryPath,
        deletePartialFilesOnConcat, TusDiskBufferSize.Default)
    {
        // Left blank.
    }
    ...
    ///     The buffer size to use when reading and writing. If unsure use
    ///     <see cref="TusDiskBufferSize.Default" />.
    /// </param>
    public TusDiskStore(string directoryPath, bool deletePartialFilesOnConcat, TusDiskBufferSize bufferSize) : this(
        directoryPath, deletePartialFilesOnConcat, bufferSize, DefaultFileIdProvider)
    {
        // Left blank.
    }
    ...
    public async Task<bool> VerifyChecksumAsync(string fileId, string algorithm, byte[] checksum, CancellationToken _)
    {
        ...
        using (var dataStream = _fileRepFactory.Data(internalFileId)
    ...
    {
        var firstLine = _fileRepFactory.UploadConcat(await InternalFileId.Parse(_fileIdProvider, fileId))
            .ReadFirstLine(true);
    ...
    /// <inheritdoc />
    public async Task<string> CreatePartialFileAsync(long uploadLength, string metadata,
        CancellationToken cancellationToken)
    {
        var fileId = await CreateFileAsync(uploadLength, metadata, cancellationToken);
        _fileRepFactory.UploadConcat(await InternalFileId.Parse(_fileIdProvider, fileId))
            .Write(new FileConcatPartial().GetHeader());
        return fileId;
    }

    /// <inheritdoc />
    public async Task<string> CreateFinalFileAsync(string[] partialFiles, string metadata,
        CancellationToken cancellationToken)
    {
        var partialInternalFileReps = new InternalFileRep[partialFiles.Length];
        for (var i = 0; i < partialFiles.Length; i++)
        {
            partialInternalFileReps[i] =
                _fileRepFactory.Data(await InternalFileId.Parse(_fileIdProvider, partialFiles[i]));
        ...
        using (var finalFile = _fileRepFactory.Data(internalFileId)
    ...
    {
        _fileRepFactory.UploadLength(await InternalFileId.Parse(_fileIdProvider, fileId))
    ...
    public async Task<string> CreateFileAsync(long uploadLength, string metadata, CancellationToken cancellationToken)
    {
        ...
        if (uploadLength != -1) await SetUploadLengthAsync(fileId, uploadLength, cancellationToken);
    ...
    {
        var firstLine = _fileRepFactory.Metadata(await InternalFileId.Parse(_fileIdProvider, fileId))
            .ReadFirstLine(true);
    ...
    {
        var expiration = _fileRepFactory.Expiration(await InternalFileId.Parse(_fileIdProvider, fileId))
            .ReadFirstLine(true);
    ...
    public async Task<IEnumerable<string>> GetExpiredFilesAsync(CancellationToken _)
    {
        ...
        foreach (var file in Directory.EnumerateFiles(_directoryPath, "*.expiration"))
        {
            ...
            if (FileHasExpired(f) && FileIsIncomplete(f)) expiredFiles.Add(f);
        }
        ...
        {
            var uploadLength = _fileRepFactory.UploadLength(fileId).ReadFirstLineAsLong(true, long.MinValue);

            if (uploadLength == long.MinValue) return true;

            var dataFile = _fileRepFactory.Data(fileId);

            if (!dataFile.Exist()) return true;
    ...
    public async Task<long> AppendDataAsync(string fileId, Stream stream, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            using var diskFileStream = _fileRepFactory.Data(internalFileId)
                .GetStream(FileMode.Append, FileAccess.Write, FileShare.None);

            var totalDiskFileLength = diskFileStream.Length;
            if (fileUploadLengthProvidedDuringCreate == totalDiskFileLength) return 0;
            ...
            do
            {
                ...
                numberOfbytesReadFromClient =
                    await stream.ReadAsync(httpReadBuffer, 0, _maxReadBufferSize, cancellationToken);
            ...
            if (!clientDisconnectedDuringRead) MarkChunkComplete(chunkCompleteFile);
    ...
    {
        var firstLine = _fileRepFactory.UploadLength(await InternalFileId.Parse(_fileIdProvider, fileId))
            .ReadFirstLine(true);
    ...
    private static async Task FlushFileToDisk(byte[] fileWriteBuffer, FileStream fileStream,
