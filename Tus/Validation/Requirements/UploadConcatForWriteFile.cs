...
internal sealed class UploadConcatForWriteFile : Requirement
{
    ...
    {
        if (!(context.Configuration.Store is ITusConcatenationStore concatStore)) return TaskHelper.Completed;
    ...
    private async Task ValidateForPatch(ContextAdapter context, ITusConcatenationStore concatStore)
    {
        ...
        if (uploadConcat is FileConcatFinal) await Forbidden("File with \"Upload-Concat: final\" cannot be patched");
    }
}