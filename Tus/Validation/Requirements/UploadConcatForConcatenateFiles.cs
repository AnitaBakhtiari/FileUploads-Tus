...
internal sealed class UploadConcatForConcatenateFiles : Requirement
{
    ...
    public UploadConcatForConcatenateFiles(UploadConcat uploadConcat, ITusConcatenationStore concatenationStore)
    {
    ...
    public override async Task Validate(ContextAdapter context)
    {
        ...
        if (_uploadConcat.Type is FileConcatFinal finalConcat) await ValidateFinalFileCreation(finalConcat, context);
    }
    ...
    private async Task ValidateFinalFileCreation(FileConcatFinal finalConcat, ContextAdapter context)
    {
        ...
        foreach (var file in finalConcat.Files)
        {
            ...
            if (length.Result != null) totalSize += length.Result.Value;

            if (length.Result != offset.Result) incompleteFiles.Add(file);
        }
