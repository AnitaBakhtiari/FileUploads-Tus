...
internal sealed class UploadLengthForCreateFileAndConcatenateFiles : Requirement
{
    public override Task Validate(ContextAdapter context)
    {
        ...
        if (uploadDeferLengthHeader == null) return VerifyRequestUploadLength(context, uploadLengthHeader);
    ...
    {
        if (deferLengthStore == null) return BadRequest($"Header {HeaderConstants.UploadDeferLength} is not supported");
    ...
    private Task VerifyRequestUploadLength(ContextAdapter context, string uploadLengthHeader)
    {
        ...
        if (uploadLengthHeader == null) return BadRequest($"Missing {HeaderConstants.UploadLength} header");

        if (!long.TryParse(request.Headers[HeaderConstants.UploadLength][0], out var uploadLength))
            return BadRequest($"Could not parse {HeaderConstants.UploadLength}");

        if (uploadLength < 0) return BadRequest($"Header {HeaderConstants.UploadLength} must be a positive number");
