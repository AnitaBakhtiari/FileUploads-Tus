...
internal sealed class UploadLengthForWriteFile : Requirement
{
    public override async Task Validate(ContextAdapter context)
    {
        ...
        if (!supportsUploadDeferLength && !uploadLengthIsSet)
            throw new TusConfigurationException(
                $"File {context.Request.FileId} does not have an upload length and the current store ({context.Configuration.Store.GetType().FullName}) does not support Upload-Defer-Length so no new upload length can be set");
    ...
    {
        var fileUploadLength =
            await context.Configuration.Store.GetUploadLengthAsync(context.Request.FileId, context.CancellationToken);
