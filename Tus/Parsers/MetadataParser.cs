...
public static class MetadataParser
{
    ...
    /// </summary>
    /// <param name="strategy">
    ///     The strategy to use when parsing. <c>MetadataParsingStrategy.Original</c> requires that the
    ///     metadata provided is composed of key value pairs while <c>MetadataParsingStrategy.AllowEmptyValues</c> allows the
    ///     use of metadata keys without values.
    /// </param>
    ...
    public static MetadataParserResult ParseAndValidate(MetadataParsingStrategy strategy,
        string uploadMetadataHeaderValue)
    {
        ...
        * */
        ...
        if (string.IsNullOrWhiteSpace(uploadMetadataHeaderValue)) return parser.GetResultForEmptyHeader();
    ...
    {
        if (strategy == MetadataParsingStrategy.Original) return new OriginalMetadataParser();
