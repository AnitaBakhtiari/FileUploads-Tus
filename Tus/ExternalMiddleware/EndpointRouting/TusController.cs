...
public abstract class TusController<TConfigurator> : ControllerBase where TConfigurator : ITusConfigurator
{
    ...
    public virtual async Task<IActionResult> FileCompleted(FileCompletedContext context, CancellationToken cancellation)
    {
        return new OkResult();
    }
    ...
    public virtual async Task<IActionResult> Download(string tusFileId)
    {
        return Ok();
    }
    ...
    internal async Task<bool> AuthorizeForAction(HttpContext context, string actionName)
    {
        ...
        if (authService != null)
        {
            var authorizeAttribute = GetType().GetMethod(actionName).GetCustomAttributes(false)
