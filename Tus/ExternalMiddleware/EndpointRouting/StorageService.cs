...
public sealed class StorageService<TTusConfigurator> where TTusConfigurator : ITusConfigurator
{
    ...
    public async Task Create(CreateContext context, CancellationToken cancellationToken)
    {
        ...
        context.FileId =
            await _storeAdapter.CreateFileAsync(context.UploadLength, context.UploadMetadata, cancellationToken);
    ...
    {
        await LoadOptions();
        return await _storeAdapter.FileExistAsync(fileId, cancellationToken);
    }
    ...
    public async Task Write(WriteContext context, CancellationToken cancellationToken)
    {
        ...
        var guardedStream = new ClientDisconnectGuardedReadOnlyStream(context.RequestStream,
            CancellationTokenSource.CreateLinkedTokenSource(cancellationToken));
        var bytesWritten =
            await _storeAdapter.AppendDataAsync(context.FileId, guardedStream, guardedStream.CancellationToken);
        ...
        if (checksum != null)
            context.ChecksumMatchesTheOneProvidedByClient = await _storeAdapter.VerifyChecksumAsync(context.FileId,
                checksum.Algorithm, checksum.Hash, cancellationToken);

        context.IsComplete = await _storeAdapter.GetUploadLengthAsync(context.FileId, cancellationToken) ==
                             context.UploadOffset;
    }
