...
public class DefaultTusConfiguration
{
    ...
    /// <summary>
    ///     The maximum upload size to allow. Exceeding this limit will return a "413 Request Entity Too Large" error to the
    ///     client.
    ...
    /// <summary>
    ///     Set the strategy to use when parsing metadata. Defaults to <see cref="MetadataParsingStrategy.AllowEmptyValues" />
    ///     for better compatibility with tus clients.
    ...
    /// <summary>
    ///     Check that the config is valid. Throws a
    ///     <exception cref="TusConfigurationException">TusConfigurationException</exception> if the config is invalid.
    /// </summary>
    internal void Validate()
    {
        if (Store == null) throw new TusConfigurationException($"{nameof(Store)} cannot be null.");
    ...
    /// <summary>
    ///     Returns the maximum upload size to allow from <see cref="MaxAllowedUploadSizeInBytesLong" /> or
    ///     <see cref="MaxAllowedUploadSizeInBytes" /> depending on which one is set.
    /// </summary>
